...
bool TerrainClass::GenerateRandomHeightMap(ID3D11Device* device, bool keydown, bool smoothing)
{
	...
	if(keydown&&(!m_terrainGeneratedToggle))
	{
 		...
 		for(int j=0; j<m_terrainHeight; j++)
		{
			...
			{			
				...
				m_heightMap[index].x = (float)i;
				m_heightMap[index].y = (float)random; //magic numbers ahoy, just to ramp up the height of the sin function so its visible.
				m_heightMap[index].z = (float)j;
			}
		}
		...
		for(int j=0; j<m_terrainHeight; j++)
		{
			...
			{			
				index = (m_terrainHeight * j) + i;
				int random = rand() % 4 + 1;
				m_heightMap[index].y = ((m_heightMap[index-4].y + m_heightMap[index-3].y + m_heightMap[index-2].y + m_heightMap[index-1].y + m_heightMap[index].y + m_heightMap[index+1].y + m_heightMap[index+2].y + m_heightMap[index+3].y + m_heightMap[index+4].y + random)/9);
			}
		}
...
bool TerrainClass::GenerateMidPointHeightMap(ID3D11Device* device, bool keydown)
{
	...
	if(keydown&&(!m_terrainGeneratedToggle))
	{
		...
		//Set intial corner values
		m_heightMap[0].y = (float)1; //Top Left
		m_heightMap[m_terrainWidth].y = (float)2; //Top Right
		m_heightMap[(m_terrainWidth * m_terrainHeight) - m_terrainWidth].y = (float)3; //Bottom Left
		m_heightMap[m_terrainWidth * m_terrainHeight].y = (float)4; //Bottom Right
